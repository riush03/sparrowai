...
function Preview({ code, device, appState, stop }: Props) {
    ...
    return (
        <div className="gap-[2.75rem] flex justify-center mx-2 w-full h-full ">
            {appState === AppState.CODING ? (
                <div
                    className={classNames(
                        'border-[4px] gap-[2.75rem] border-black rounded-[20px] shadow-lg w-full',
                        'transform scale-[0.9] origin-content',
                        {
                            'w-full h-[700px] gap-[2.75rem]  flex': device === 'desktop',
                            'w-[400px] h-[700px] gap-[2.75rem]': device === 'mobile',
                        }
                    )}
                >
